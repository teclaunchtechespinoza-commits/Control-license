...
async def login(user_credentials: UserLogin, response: Response, request: Request):
...
async def reset_user_password_new(
    ...
    # Verificar se é admin ou super_admin (comparar com enum ou string)
...
async def update_pessoa_fisica(
    ...
    """Update Cliente PF with tenant isolation"""
    # 🔧 FIX: Usar tenant_id diretamente
...
async def get_permissions(current_user: User = Depends(get_current_user)):
    # Apenas admins e super_admins podem ver permissões
    user_role_str = current_user.role.value if hasattr(current_user.role, 'value') else str(current_user.role)
    if user_role_str not in ["admin", "super_admin"]:
        raise HTTPException(status_code=403, detail="Acesso negado. Apenas administradores.")
    ...
    try:
        ...
        # Tentar buscar cliente PJ
            ...
            if client_pj:
                ...
                # Priorizar WhatsApp, depois telefone_principal, depois celular, depois telefone
                client_phone = (client_pj.get('whatsapp') or 
                              client_pj.get('telefone_principal') or 
                              client_pj.get('celular') or 
                              client_pj.get('telefone'))
...
async def get_stats(
    ...
    if current_user.role == "super_admin":
        ...
        try:
            ...
            # Contar licenças sem data (consideradas ativas)
            no_date_licenses = await db.licenses.count_documents({"expires_at": None})
            # Contar licenças sem campo (consideradas ativas)
        ...
        try:
            # Contar licenças não expiradas
            future_licenses = await db.licenses.count_documents(add_tenant_filter({"expires_at": {"$gte": now}}, tenant_id))
            # Contar licenças sem data (consideradas ativas)
            no_date_licenses = await db.licenses.count_documents(add_tenant_filter({"expires_at": None}, tenant_id))
            # Contar licenças sem campo (consideradas ativas)
            no_field_licenses = await db.licenses.count_documents(add_tenant_filter({"expires_at": {"$exists": False}}, tenant_id))
            
            active_licenses = future_licenses + no_date_licenses + no_field_licenses
            logger.info(f"DEBUG (tenant): future={future_licenses}, no_date={no_date_licenses}, no_field={no_field_licenses}, total_active={active_licenses}")
        except Exception as e:
